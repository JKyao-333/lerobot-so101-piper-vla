...
The wrapper accepts `--suite spatial|object|goal|long`, a local checkpoint path or Hugging Face repository id, output directory, and center-crop boolean. It verifies the OpenVLA and LIBERO roots, imports LIBERO/MuJoCo, requires headless EGL when no display exists, and creates a suite-specific timestamped run directory. The manual profile maps the four suites to the corresponding `openvla/openvla-7b-finetuned-libero-*` ids.
