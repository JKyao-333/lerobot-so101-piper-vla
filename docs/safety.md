...
Any invalid action fails fast: the first dimension, numeric, finite-value, absolute-bound, or step-delta violation enters `ABORTED` and attempts a best-effort position hold. That hold is not an emergency stop. It may fail when the control channel or upstream private interface is unavailable, and such failure is logged without replacing the original fault.
