...
The Piper wrapper maps `observation.images.front` and `observation.images.wrist` to the policy's `camera1` and `camera2` keys. The manual reference run uses batch size 4, 4 workers, 20,000 steps, and a 5,000-step save frequency. Base model, SmolVLM path, dataset, output, learning rate, device, offline mode, resume checkpoint, and LoRA settings remain runtime inputs.
