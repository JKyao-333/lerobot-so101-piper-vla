...
The Piper wrapper maps `observation.images.front` and `observation.images.wrist` to the policy's `camera1` and `camera2` keys. Base model, SmolVLM path, dataset, output, batch size, learning rate, steps, save frequency, device, offline mode, resume checkpoint, and LoRA settings are runtime inputs.
